...
class Ion_auth_model extends CI_Model
{
	...
	public function like($like, $value = NULL, $position = 'both')
	{
		...
		$this->_ion_like = array_merge($this->_ion_like, $like);
	 ...
	 **/
        public function users($groups = NULL){
            ...
            $query = "SELECT "; $selected = ""; $distinct = ""; $grouping = ""; $where = "";
            //Add selected fields
            ...
            }else $selected .= "users.*, users.id as id, users.id as user_id ";
            //filter by group id(s) if passed
            if(!empty($groups)){
                $distinct = "DISTINCT ";
                $grouping .= "INNER JOIN users_groups ON users_groups.user_id = users.id ";
                if(!is_array($groups)) $groups = array($groups);
                if(!is_numeric(reset($groups))) $grouping .= "INNER JOIN groups ON users_groups.group_id = groups.id WHERE groups.name IN ( '" . implode("', '", $groups) . "' ) ";
                else $grouping .= "WHERE users_groups.group_id IN ( " . implode(", ", $groups) . " ) ";
            }
            $query = $query . $distinct . $selected . "FROM users " . $grouping;
            //run each where that was passed
            $this->trigger_events('extra_where');
            if (!empty($this->_ion_where)){
                $where = TRUE;
                if(!empty($groups)) $query .= "AND ";
                else $query .= "WHERE ";
                $first = TRUE;
                foreach ($this->_ion_where as $wh){
            ...
            //run each likes that was passed
            if(!empty($this->_ion_like)){
                if(!empty($groups) || !empty($where)) $query .= "AND ( ";
                else $query .= "WHERE ( ";
                $first = TRUE;
                foreach ($this->_ion_like as $column => $value){
                    if($first) $first = FALSE;
                    else $query .= "OR ";
                    $value = preg_replace('~[\x00\x0A\x0D\x1A\x22\x25\x27\x5C\x5F]~u', '\\\$0', $value);
                    $query .= "users." . $column . " LIKE '%" . $value . "%' ESCAPE '!' ";
                }
                $query .= ")";
            ...
            if(isset($this->_ion_order_by) && isset($this->_ion_order)){
                $query .= "ORDER BY " . $this->_ion_order_by . " " . $this->_ion_order . " ";
                $this->_ion_order    = NULL;
                $this->_ion_order_by = NULL;
            }
            //set limit and offset for pagination or other purpose
            if(isset($this->_ion_limit) && isset($this->_ion_offset)){
                $query .= "LIMIT " . $this->_ion_offset . ", " . $this->_ion_limit . "";
	...
	public function users11($groups = NULL){
		$this->trigger_events('users');
		...
		{
                        $first = TRUE;
			foreach ($this->_ion_like as $like => $key)
			{
                                if($first){
                                    $first = FALSE;
                                    $this->db->like($like, $key);
                                }
				$this->db->or_like($like, $key);
			}
	...
	public function count_users($groups = NULL){
            $query = "SELECT COUNT(*) AS numrows FROM ( SELECT DISTINCT users.id as i FROM users ";
            //filter by group id(s) if passed
            if(!empty($groups)){
                $query .= "INNER JOIN users_groups ON users_groups.user_id = users.id ";
                if(!is_array($groups)) $groups = array($groups);
                if(!is_numeric(reset($groups))) $query .= "INNER JOIN groups ON users_groups.group_id = groups.id WHERE groups.name IN ( '" . implode("', '", $groups) . "' ) ";
                else $query .= "WHERE users_groups.group_id IN ( " . implode(", ", $groups) . " ) ";
            }
            //run each where that was passed
            if (!empty($this->_ion_where)){
                if(!empty($groups)) $query .= "AND ";
                ...
                foreach ($this->_ion_where as $wh){
                        ...
                        $query .= "users." . $column . " = '" . $value . "' ";
                    }
                }
            }
            //run each likes that was passed
            if (!empty($this->_ion_like)){
                if(!empty($groups) || !empty($this->_ion_where)) $query .= "AND ( ";
                else $query .= "WHERE ( ";
                $first = TRUE;
                foreach ($this->_ion_like as $column => $value){
                    if($first) $first = FALSE;
                    else $query .= "OR ";
                    $value = preg_replace('~[\x00\x0A\x0D\x1A\x22\x25\x27\x5C\x5F]~u', '\\\$0', trim($value));
                    $query .= "users." . $column . " LIKE '%" . $value . "%' ESCAPE '!' ";
                }
                $query .= ")";
            }
            ...
            return $this->db->query($query)->row()->numrows;
	}
